...
export const requireRole = (role: 'agent' | 'compliance' | 'admin'): RequestHandler => {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (!dbUser || dbUser.status !== 'active') {
        return res.status(401).json({ message: "Unauthorized - inactive user" });
      }

      // Role hierarchy: admin > compliance > agent
      const roleHierarchy = { 'agent': 1, 'compliance': 2, 'admin': 3 };
      const requiredLevel = roleHierarchy[role];
      const userLevel = roleHierarchy[dbUser.role];
